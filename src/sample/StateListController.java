...
public class StateListController implements Initializable {
    ...
    public void view() {
        try {
            ...
            ResultSet rs = stmt.executeQuery("SELECT SL.stateid, SL.state_name, SL.Active FROM StateList SL");
    ...
    public void Add() {
        try {
            data = FXCollections.observableArrayList();
            PreparedStatement pstmt = conn.prepareStatement("INSERT INTO StateList (state_name, Active) Values (?,?)");
    ...
    public void Update(ActionEvent actionEvent) throws SQLException {
        ...
        PreparedStatement pstmt = conn.prepareStatement("UPDATE StateList SET state_name = ?, Active = ? WHERE stateid = " + ID );
        pstmt.setString(1, StatusTextfield.getText());
        pstmt.setString(2, IsActiveCheckbox.isSelected() ? "true":"false");
        pstmt.executeUpdate();
