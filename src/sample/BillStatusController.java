...
public class BillStatusController implements Initializable {
    ...
    public void view() {
        try {
            ...
            ResultSet rs = stmt.executeQuery("SELECT BS.billstatusid, BS.bill_status, BS.Active FROM BillStatus BS");
    ...
    public void Add() {
        try {
            data = FXCollections.observableArrayList();
            PreparedStatement pstmt = conn.prepareStatement("INSERT INTO BillStatus (bill_status, Active) Values (?,?)");
    ...
    public void Update(ActionEvent actionEvent) throws SQLException {
        ...
        PreparedStatement pstmt = conn.prepareStatement("UPDATE BillStatus SET bill_status = ?, Active = ? WHERE billstatusid = " + ID );
        pstmt.setString(2, IsActiveCheckbox.isSelected() ? "true":"false");
        pstmt.setString(1, StatusTextfield.getText());
        pstmt.executeUpdate();
