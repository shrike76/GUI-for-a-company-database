...
public class StreetListController implements Initializable {
    ...
    public void view() {
        try {
            ...
            ResultSet rs = stmt.executeQuery("SELECT STL.streetid, STL.street_name, STL.Active FROM StreetList STL");
    ...
    public void Add() {
        try {
            data = FXCollections.observableArrayList();
            PreparedStatement pstmt = conn.prepareStatement("INSERT INTO StreetList (street_name, Active) Values (?,?)");
    ...
    public void Update(ActionEvent actionEvent) throws SQLException {
        ...
        PreparedStatement pstmt = conn.prepareStatement("UPDATE StreetList SET street_name = ?, Active = ? WHERE streetid = " + ID );
        pstmt.setString(1, StatusTextfield.getText());
        pstmt.setString(2, IsActiveCheckbox.isSelected() ? "true":"false");
        pstmt.executeUpdate();
