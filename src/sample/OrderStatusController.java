...
public class OrderStatusController implements Initializable {
    ...
    public void view() {
        try {
            ...
            ResultSet rs = stmt.executeQuery("SELECT OS.orderstatusid, OS.order_status, OS.Active FROM OrderStatus OS");
    ...
    public void Add() {
        try {
            data = FXCollections.observableArrayList();
            PreparedStatement pstmt = conn.prepareStatement("INSERT INTO OrderStatus (order_status, Active) Values (?,?)");
    ...
    public void Update(ActionEvent actionEvent) throws SQLException {
        ...
        PreparedStatement pstmt = conn.prepareStatement("UPDATE OrderStatus SET order_status = ?, Active = ? WHERE orderstatusid = " + ID );
        pstmt.setString(1, StatusTextfield.getText());
        pstmt.setString(2, IsActiveCheckbox.isSelected() ? "true":"false");
        pstmt.executeUpdate();
